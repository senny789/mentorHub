...
import { addon, form, period, plan } from "../../../pages/multi-step-form";
...
const Summary: React.FC<SummaryProps> = ({
	...
	const totalAmount = addon?.reduce((prev, add) => {
			...
			<div className="grid grid-cols-1 gap-4 ">
				<div className=" bg-slate-100 p-5 rounded-md flex flex-col gap-6">
					<section className="font-bold flex justify-between">
						...
						<span>
							${cost.plan[plan ?? "arcade"][period ?? "monthly"]}/
				...
				<div className="flex justify-between font-bold ">
					...
					<span className="text-[hsl(243,100%,62%)]">
						${totalAmount}/{period === "monthly" ? "mth" : "yr"}
					</span>
				</div>
			</div>
		</div>
